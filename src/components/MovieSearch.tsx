...
const MovieSearch: FC = () => {
  ...
  const fetchSearch: MouseEventHandler = async (e) => {
    ...
    try {
      const data = await fetch(
        `https://api.themoviedb.org/3/search/movie?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=ja&query=${text}`
      );
  ...
  return (
    <div className="h-screen relative top-10   ">
      <p className="text-center font-bold text-xl relative top-3">映画検索</p>
      <div className="relative top-7 flex justify-center gap-x-6">
        <Button variant="text" onClick={() => setSearch(true)}>
          TV検索
        </Button>
      ...
      <div className="relative top-14 gap-2 mx-2  grid  grid-cols-2  sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5   ">
        {show?.map((shows) => {
          return (
            <div
              onClick={() => {
